  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "conversations", id)).then((snap) => {
      if (snap.exists()) {
        ...
        if (otherId) {
          getDoc(doc(db, "users", otherId)).then((u) => {
            if (u.exists()) setOtherUser(u.data() as UserProfile);
          });
        }
      }
    });
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <View style={[styles.msgRow, isMine ? styles.msgRowMine : styles.msgRowOther]}>
        {!isMine && (
          <Avatar uri={otherUser?.photoURL} name={otherUser?.displayName} size={28} />
        )}
        <View
          style={[
            styles.bubble,
            {
              backgroundColor: isMine ? colors.bubble : colors.bubbleOther,
        ...
        >
          {item.imageUrl ? (
            ...
            <View style={[styles.videoPlaceholder, { backgroundColor: colors.secondary }]}>
              ...
              <Text style={[styles.videoText, { color: colors.foreground }]}>Video</Text>
            </View>
          ) : (
          ...
          <Text style={[styles.msgTime, { color: isMine ? "rgba(255,255,255,0.6)" : colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 10, backgroundColor: colors.headerBg, borderBottomColor: colors.border }]}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Avatar uri={otherUser?.photoURL} name={otherUser?.displayName} size={36} isVerified={otherUser?.isVerified} />
          <View>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
              <Text style={[styles.headerName, { color: colors.foreground, fontFamily: "Inter_600SemiBold" }]}>
                {otherUser?.displayName || "Yükleniyor..."}
              </Text>
              {otherUser?.isAdmin && <BadgeTag type="admin" size="sm" />}
            </View>
...
const styles = StyleSheet.create({
  ...
  videoPlaceholder: { width: 220, height: 120, borderRadius: 10, alignItems: "center", justifyContent: "center", gap: 8 },
